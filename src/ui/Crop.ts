...
export class Crop extends Modal {
	...
	async onOpen() {
		...
		modalHeader.createEl('h2', { text: t("MODAL_CROP_TITLE") });
		...
		[
			{ name: 'free', ratio: null, label: t("LABEL_CROP_FREE") },
	...
	private createTransformControls(modalHeader: HTMLElement) {
		...
		const rotateLeftBtn = rotateContainer.createEl('button', {
			...
			text: '↺',
			attr: { title: t("TOOLTIP_CROP_ROTATE_CCW") }
		});

		const rotateRightBtn = rotateContainer.createEl('button', {
			cls: 'transform-button',
			text: '↻',
			attr: { title: t("TOOLTIP_CROP_ROTATE_CW") }
		});
		...
		const flipHorizontalBtn = flipContainer.createEl('button', {
			cls: 'transform-button',
			text: '↔',
			attr: { title: t("TOOLTIP_CROP_FLIP_H") }
		});

		const flipVerticalBtn = flipContainer.createEl('button', {
			cls: 'transform-button',
			text: '↕',
			attr: { title: t("TOOLTIP_CROP_FLIP_V") }
		});
	...
	private createImageControls(modalHeader: HTMLElement) {
		...
		rotationContainer.createEl('span', { text: t("LABEL_CROP_ROTATION") + ": ", cls: 'control-label' });
		...
		zoomContainer.createEl('span', { text: t("LABEL_CROP_ZOOM") + ": ", cls: 'control-label' });
